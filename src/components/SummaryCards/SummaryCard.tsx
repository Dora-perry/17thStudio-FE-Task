...
export default function SummaryCard({
  ...
  change,
  isLoading,
}: SummaryCardProps & { isLoading?: boolean }) {
  ...
  return (
    <div className="w-full rounded-[20px] p-4 md:p-6 bg-[#E5E8E8] hover:shadow-md hover:scale-[1.01] transition-all duration-200">
      <div className="flex justify-between items-center mb-2">
      ...
      <p
        className={`text-sm md:text-base font-medium ${
          isPositive ? 'text-[#3E7383]' : 'text-[#3E7383]'
        }`}
      >
        {isPositive ? '+' : ''}
        {change}%
